...
import {
  X, Search, Braces, Hash, ChevronRight, ChevronDown,
  ExternalLink, FileCode, Copy, Layers, GitBranch,
  Circle, Zap, List, AlignLeft, Tag,
} from "lucide-react";
...
export function ApiReferenceViewer({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const parentCls = cls.inherits ? CLASSES.find(c => c.name === cls.inherits) : null;
      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center"
          ...
          <motion.div initial={{opacity:0,scale:0.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.97,y:12}}
            ...
            style={{width:"min(1040px,97vw)",height:"min(700px,90vh)",background:T.bgElevated,border:`1px solid ${T.border}`,boxShadow:`0 40px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.04)`}}>
