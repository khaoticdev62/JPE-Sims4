...
import {
  ...
  ArrowRight, CornerDownRight, Clock, LayoutGrid, BookOpen,
  type LucideIcon,
} from "lucide-react";
...
type CommandCategory = "translation" | "build" | "navigation" | "search" | "analysis" | "debug" | "editor" | "general";
...
/* ═══ MAIN COMPONENT ═══ */
export function CommandPalette({ isOpen, onClose, onSwitchMode, currentMode }: CommandPaletteProps) {
  ...
  if (isCommandMode) {
    ...
    const catMap = new Map<CommandCategory, typeof filteredItems>();
    rest.forEach(item => {
      if (item.type === "command" && item.cmd) {
  ...
  return (
    ...
    <div className="absolute inset-0 z-[100] flex items-start justify-center pt-[10vh]" onClick={onClose}>
      ...
      <motion.div
        ...
        style={{ background: "rgba(7,8,16,0.78)", backdropFilter: T.glassBlur }}
      />
