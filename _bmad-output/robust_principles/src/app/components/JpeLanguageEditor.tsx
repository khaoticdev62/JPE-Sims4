...
import {
  BookOpen, Code2, Lightbulb, Play, Copy, CheckCircle2,
  ChevronRight, Search, FileCode, Sparkles, AlertTriangle,
  ArrowRight, Wand2, Eye, type LucideIcon,
} from "lucide-react";
import { T } from "../pages/jpe-theme";
import { Eyebrow, Badge, PanelHeader, IconBtn, GlowDot } from "../pages/jpe-shared";
import {
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ borderRight: `1px solid ${T.border}` }}>
          ...
          <div className="flex-1 overflow-y-auto py-2 relative">
            ...
            {/* Autocomplete popup */}
            {showSuggestions && (
              <div className="absolute left-16 rounded-xl overflow-hidden z-10"
                style={{ top: (selectedLine || 0) * 22 + 40, background: T.bgSurface, border: `1px solid ${T.border}`, boxShadow: `0 8px 32px rgba(0,0,0,0.5)`, minWidth: 280 }}>
                ...
                {jpeSuggestions.slice(0, 6).map((s, i) => (
                  <div key={i} className="flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors"
                    ...
                    <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: `${s.color || T.violet}15` }}>
                      <Code2 size={10} color={s.color || T.violet} />
                    </div>
                    <span style={{ fontSize: 11, fontFamily: T.mono, color: T.textPrimary, fontWeight: 600 }}>{s.label}</span>
                    <span className="ml-auto" style={{ fontSize: 9, fontFamily: T.mono, color: T.textDim }}>{s.type}</span>
                  </div>
                ))}
              </div>
            )}
