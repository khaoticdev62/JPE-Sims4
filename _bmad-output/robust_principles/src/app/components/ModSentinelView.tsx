...
import {
  ...
  Info, ChevronDown, ChevronRight, X, Zap, Clock,
  FileCode, Lock, Unlock, Eye, Copy, ExternalLink,
  CheckCircle2, XCircle, MoreHorizontal, Sparkles,
  type LucideIcon,
} from "lucide-react";
...
const severityConfig: Record<Severity, { color: string; bg: string; borderColor: string; glowColor: string; priorityLabel: string; priorityColor: string }> = {
  CRITICAL: { color: T.rose, bg: "rgba(244,63,94,0.06)", borderColor: "rgba(244,63,94,0.15)", glowColor: "rgba(244,63,94,0.3)", priorityLabel: "High Priority", priorityColor: T.rose },
  ALERT: { color: T.amber, bg: "rgba(245,158,11,0.05)", borderColor: "rgba(245,158,11,0.12)", glowColor: "rgba(245,158,11,0.2)", priorityLabel: "Moderate", priorityColor: T.amber },
  HINT: { color: T.cyan, bg: "rgba(6,182,212,0.04)", borderColor: "rgba(6,182,212,0.1)", glowColor: "rgba(6,182,212,0.15)", priorityLabel: "Low", priorityColor: T.cyan },
};
...
const errors: DiagnosticError[] = [
  { id: "E001", name: "SYSTEM_CRASH_E01", severity: "CRITICAL", source: "Glow", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "core_system.cpp", line: 142, description: "Buffer overflow in memcpy destination" },
  { id: "E002", name: "DATA_CORRUPTION", severity: "CRITICAL", source: "Glow", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "data_handler.cpp", line: 87, description: "Corrupted memory block at 0x0FF0" },
  { id: "E003", name: "DATA_CORRUPTION", severity: "CRITICAL", source: "Glow", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "io_stream.cpp", line: 203, description: "Invalid checksum on data block" },
  { id: "E004", name: "NULL_REFERENCE", severity: "CRITICAL", source: "Glow", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "core_system.cpp", line: 156, description: "Null pointer dereference in handler" },
  { id: "E005", name: "MODULE_TIMEOUT", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "module_loader.cpp", line: 44, description: "Module init exceeded 30s timeout" },
  { id: "E006", name: "AUTH_FAILURE", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "auth_service.cpp", line: 112, description: "Token validation failed" },
  { id: "E007", name: "DUBLE_TIMEOUT", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "network_handler.cpp", line: 78, description: "Duplicate timeout on socket" },
  { id: "E008", name: "DATA_MASK", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "data_mask.cpp", line: 33, description: "Masking failed on sensitive field" },
  { id: "E009", name: "HEAP_WARNING", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "memory_mgr.cpp", line: 201, description: "Heap fragmentation above threshold" },
  { id: "E010", name: "RATE_LIMIT", severity: "ALERT", source: "Alert", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "api_gateway.cpp", line: 55, description: "Rate limit exceeded on endpoint" },
  { id: "E011", name: "OPTIMIZATION_SUGGESTION", severity: "HINT", source: "Low", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "render_pipeline.cpp", line: 310, description: "Loop can be vectorized" },
  { id: "E012", name: "USET", severity: "HINT", source: "Low", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "config_parser.cpp", line: 22, description: "Unused variable detected" },
  { id: "E013", name: "OPARHIER", severity: "HINT", source: "Low", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "hierarchy.cpp", line: 67, description: "Operator hierarchy ambiguous" },
  { id: "E014", name: "DEAD_CODE", severity: "HINT", source: "Low", timestamp: "22:18:45", kernel: "Kernel_V4", status: "UNRESOLVED", file: "legacy_api.cpp", line: 144, description: "Unreachable code after return" },
];
...
/* ═══ SECTION HEADER ═══ */
function SectionHeader({ children, icon: Icon, iconColor = T.textTertiary, collapsible = false, defaultOpen = true }: { children: React.ReactNode; icon?: LucideIcon; iconColor?: string; collapsible?: boolean; defaultOpen?: boolean }) {
...
export function ModSentinelView() {
  ...
  const alerts = errors.filter((e) => e.severity === "ALERT");
  const hints = errors.filter((e) => e.severity === "HINT");
  const activeError = errors[selectedError];
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex flex-col flex-shrink-0" style={{ width: errListW, borderRight: `1px solid ${T.border}`, background: T.bgPanel }}>
          <div className="flex items-center justify-between px-3 py-2 flex-shrink-0" style={{ borderBottom: `1px solid ${T.border}` }}>
            <span style={{ fontSize: 11, fontWeight: 700, fontFamily: T.display, color: T.textPrimary }}>ACTIVE ERROR LIST ({errors.length})</span>
            <button className="p-1 rounded hover:bg-white/5" onClick={() => { navigator.clipboard.writeText(errors.map(e => `[${e.severity}] ${e.name}: ${e.description}`).join("\n")).then(() => toast.success("Error list copied")).catch(() => {}); }} title="Copy error list"><MoreHorizontal size={12} color={T.textTertiary} /></button>
          </div>
          ...
          <div className="px-3 py-1.5 flex items-center justify-between flex-shrink-0" style={{ borderTop: `1px solid ${T.border}` }}>
            <span style={{ fontSize: 8, fontFamily: T.mono, color: T.textMuted }}>{errors.filter((e) => e.status === "UNRESOLVED").length} unresolved</span>
            <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ background: T.bg }}>
          ...
          <div className="flex items-center justify-between px-4 py-2 flex-shrink-0" style={{ borderBottom: `1px solid ${T.border}` }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="p-1 rounded hover:bg-white/5" onClick={() => setSelectedError(p => (p + 1) % errors.length)} title="Next error"><ExternalLink size={10} color={T.textTertiary} /></button>
            </div>
          </div>
                    ...
                    <div
                      ...
                      style={{
                        background: line.type === "error"
                          ...
                          : "transparent",
                        borderLeft: line.type === "error" ? `3px solid ${T.rose}` : line.type === "context" ? `3px solid ${T.amber}` : "3px solid transparent",
                      }}
                    >
