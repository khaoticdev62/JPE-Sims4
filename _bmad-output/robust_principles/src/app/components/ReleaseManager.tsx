...
import {
  X, Rocket, Tag, Check, AlertTriangle, XCircle,
  RefreshCw, Copy, Download, Globe, Github, Package,
  Plus, Minus, ChevronRight, FileText, Zap, Lock,
  CheckCircle2, Upload,
} from "lucide-react";
...
function parseVer(s: string): SemVer {
  const [ma,mi,pa] = s.split(".").map(Number);
      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center"
          ...
          <motion.div initial={{opacity:0,scale:0.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.97,y:12}}
            ...
            style={{width:"min(1020px,97vw)",height:"min(720px,92vh)",background:T.bgElevated,border:`1px solid ${T.border}`,boxShadow:`0 40px 80px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.04)`}}>
