...
from jpe_sims4.ts4rebels.translation_pack import (
    apply_translation_pack,
    load_translation_pack,
)
from jpe_sims4.ts4rebels.validation_preset import apply_ts4rebels_validation_preset

from jpe_sims4.better_exceptions.collector import ExceptionCollector
from jpe_sims4.better_exceptions.parser import ExceptionParser
from jpe_sims4.better_exceptions.resolver import ModResolver
from jpe_sims4.better_exceptions.analyzer import RootCauseAnalyzer
...
JPE_HEADER = r"""
   ...
   ██║██║      ███████╗    ███████╗   ██║   ╚██████╔╝██████╔╝██║╚██████╔╝
   ╚═╝╚═╝      ╚══════╝    ╚══════╝   ╚═╝    ╚═════╝ ╚═════╝ ╚═╝ ╚═════╝ 
                    INDUSTRIAL MODDING TOOLCHAIN v2.1
"""
...
def _cmd_extract(args: argparse.Namespace) -> int:
    ...
    if args.json or not args.write:
        sys.stdout.write(json.dumps(payload, indent=2, ensure_ascii=False) + "\n")
    return _exit_code_for_diagnostics(
        result.project.diagnostics, warnings_as_errors=bool(args.warnings_as_errors)
    )
...
def _cmd_build(args: argparse.Namespace) -> int:
    ...
    if args.report:
        Path(args.report).write_text(
            render_diagnostics_markdown(project, res.diagnostics), encoding="utf-8"
        )
    ...
    exit_code = _exit_code_for_diagnostics(
        res.diagnostics, warnings_as_errors=bool(args.warnings_as_errors)
    )
...
def _cmd_report(args: argparse.Namespace) -> int:
    ...
    exit_code = _exit_code_for_diagnostics(
        diagnostics, warnings_as_errors=bool(args.warnings_as_errors)
    )
...
def _cmd_validate(args: argparse.Namespace) -> int:
    ...
    diags = validate_project_segments(
        project.segments, glossary_entries=project.glossary, rules=project.validation
    )
    diagnostics = [d.to_dict() for d in diags]
    exit_code = _exit_code_for_diagnostics(
        diags, warnings_as_errors=bool(args.warnings_as_errors)
    )
    payload = {
    ...
    if args.update_project:
        keep = [
            d
            for d in (project.diagnostics or [])
            if str(d.get("category") or "") not in {"VALIDATION", "GLOSSARY"}
        ]
...
def _cmd_export_csv(args: argparse.Namespace) -> int:
    ...
    if args.json:
        sys.stdout.write(
            json.dumps({"out": str(out), "segments": len(project.segments)}, indent=2)
            + "\n"
        )
    return 0
...
def _cmd_glossary(args: argparse.Namespace) -> int:
    ...
    if args.action == "export":
        ...
        if args.json:
            sys.stdout.write(
                json.dumps(
                    {"out": str(out), "entries": len(project.glossary)}, indent=2
                )
                + "\n"
            )
        return 0
    ...
    if args.action == "import":
        ...
        res = import_glossary_csv(
            project, Path(args.csv), overwrite=(not args.no_overwrite)
        )
        if args.in_place:
            save_project(project, project_path)
        elif args.write:
        ...
        if args.json:
            ...
            sys.stdout.write(
                f"Glossary entries: {len(project.glossary)} (added={res.added}, updated={res.updated}, skipped={res.skipped})\n"
            )
        return 0
...
def _cmd_plugins(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        sys.stdout.write(
            f"Extractors: {', '.join(sorted(reg.extractors.keys())) or '(none)'}\n"
        )
        sys.stdout.write(
            f"Appliers: {', '.join(sorted(reg.appliers.keys())) or '(none)'}\n"
        )
...
def _cmd_sync(args: argparse.Namespace) -> int:
    ...
    if args.action in {"push", "status", "merge"}:
        ...
        if args.action == "push":
            ...
            remote_proj, diags_pull = pull(
                project_uid=str(args.project_uid), sync_root=root
            )
                    ...
                    json.dumps(
                        {
                            ...
                            "diagnostics": [d.to_dict() for d in diags_pull],
                        },
                        indent=2,
                        ensure_ascii=False,
                    )
    ...
    try:
        ...
        if db_path.exists():
            ...
            for h in hits:
                ...
                seen.add(t)
                preds.append(
                    Prediction(score=int(h.score), target=t, reason="tm_sqlite")
                )
                if len(preds) >= limit:
                    break
    except Exception:
...
def _tm_pair_from_project(
    project: Project, args: argparse.Namespace
) -> tuple[str, str]:
    src = (
        str(
            getattr(args, "source_locale", "") or project.source_locale or "und"
        ).strip()
        or "und"
    )
    tgt = (
        str(
            getattr(args, "target_locale", "") or project.target_locale or "und"
        ).strip()
        or "und"
    )
    return src, tgt
...
def _cmd_tm(args: argparse.Namespace) -> int:
    ...
    db_path = (
        ...
        else default_tm_db_path(project_path)
    )
    store = SqliteTMStore(db_path=db_path)

    if args.tm_action == "migrate":
        added = store.ingest_segments(
            project.segments, source_locale=src_loc, target_locale=tgt_loc
        )
        payload = {
            "action": "migrate",
            "db": str(db_path),
    ...
    if args.tm_action == "import":
        ...
        if fmt == "csv":
            added = store.import_csv(
                in_path, source_locale=src_loc, target_locale=tgt_loc
            )
        elif fmt == "json":
            added = store.import_json(
                in_path, source_locale=src_loc, target_locale=tgt_loc
            )
        else:
            raise SystemExit("Unknown --format (use json or csv).")
    ...
    if args.tm_action == "suggest":
        ...
        payload = {
            ...
            "db": str(db_path),
            "source": str(args.source or ""),
            "suggestions": [h.__dict__ for h in hits],
        }
        sys.stdout.write(json.dumps(payload, indent=2, ensure_ascii=False) + "\n")
        return 0
...
def _cmd_import_csv(args: argparse.Namespace) -> int:
    ...
    payload = {
        "updated_segments": res.updated_segments,
        "diagnostics": [d.to_dict() for d in res.diagnostics],
    }
...
def _cmd_bulk(args: argparse.Namespace) -> int:
    ...
    if args.action == "propagate":
        res = propagate_targets_by_source_scoped(
            project.segments,
            scope=scope,
        ...
        if bool(getattr(args, "sqlite_tm", False)):
            try:
                db_path = (
                    Path(args.tm_db).expanduser()
                    if getattr(args, "tm_db", None)
                    else default_tm_db_path(project_path)
                )
                store = SqliteTMStore(db_path=db_path)
                src_loc = str(project.source_locale or "und").strip() or "und"
...
def _ts4rebels_client_from_args(args: argparse.Namespace) -> TS4RebelsClient:
    base = (
        str(getattr(args, "base_url", "") or "").strip() or "https://ts4rebels.cc/"
    )
    config = ClientConfig(
        base_url=base,
        enable_network=bool(getattr(args, "enable_network", False)),
        allowed_hosts=tuple(
            h.strip()
            for h in str(getattr(args, "allowed_hosts", "ts4rebels.cc"))
            .replace(";", ",")
            .split(",")
            if h.strip()
        ),
    ...
    # Re-inject session cookies if provided (e.g. from a previous login)
...
def _cmd_ts4rebels_login(args: argparse.Namespace) -> int:
    user = str(args.username or os.environ.get("JPE_TS4REBELS_USER") or "").strip()
    pw = str(args.password or os.environ.get("JPE_TS4REBELS_PASS") or "")
    client = _ts4rebels_client_from_args(args)
    diags = client.login(username=user, password=pw)

    # Capture cookies even if login failed (for potential debugging/CSRF)
    cookies = client.get_cookies()

    res = {
        "ok": not diags,
        "diagnostics": [(d.to_dict() if hasattr(d, "to_dict") else d) for d in diags],
        "cookies": cookies if not diags else {},
    }
    sys.stdout.write(json.dumps(res, indent=2, ensure_ascii=False) + "\n")
    return _exit_code_for_diagnostics(diags)
...
def _cmd_ts4rebels_forum(args: argparse.Namespace) -> int:
    ...
    if not bool(getattr(args, "enable_network", False)):
        diags = [
            ts4di.network_disabled(
                message="Network is disabled. Pass --enable-network to use TS4Rebels commands."
            )
        ]
    ...
    topics, diags = client.list_forum_topics(
        forum_id=int(args.forum_id), page=int(args.page)
    )
    payload = {
        ...
        "posts": [
            {
                ...
                "links": [link.__dict__ for link in p.links],
            }
            for p in posts
        ],
        "diagnostics": [d.to_dict() for d in diags],
...
def _cmd_ts4rebels_download_import(args: argparse.Namespace) -> int:
    ...
    dl_hosts = tuple([h.strip() for h in allowed_dl.split(",") if h.strip()]) or (
        "ts4rebels.cc",
    )
...
def _cmd_diagnose(args: argparse.Namespace) -> int:
    ...
    if args.logs:
        ...
        logs = collector.find_logs()

    if not logs:
        sys.stdout.write("No logs found.\n")
        return 0

    # For now, analyze the latest log or specified logs
    reports = []
    for log_path in logs[: int(args.limit)]:
        raw_content = collector.read_log(log_path)
        exception = parser.parse(raw_content)
        suspected_mods = resolver.resolve(exception)
        report = analyzer.analyze(exception, suspected_mods)
    ...
    if args.json:
        ...
        for r in reports:
            sys.stdout.write(f"\n--- Diagnostic Report ({r.report_id}) ---\n")
            sys.stdout.write(f"Time: {r.timestamp}\n")
            sys.stdout.write(f"Error: {r.error_summary}\n")
            sys.stdout.write(f"Severity: {r.severity.value}\n")
            sys.stdout.write(f"Root Cause: {r.root_cause}\n")
            if r.suspected_mods:
                sys.stdout.write(
                    f"Suspected Mods: {', '.join(m.mod_name for m in r.suspected_mods)}\n"
                )
            sys.stdout.write("Recommended Fixes:\n")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan = sub.add_parser(
        "scan", help="Scan a folder or zip and produce a project JSON."
    )
    scan.add_argument("path", help="Path to a mod folder or .zip.")
    scan.add_argument("--write", help="Write project JSON to a file path.")
    scan.add_argument(
        "--merge-from", help="Previous project JSON to reuse file hashes from."
    )
    scan.add_argument(
        "--json", action="store_true", help="Print project JSON to stdout."
    )
    scan.add_argument(
        "--warnings-as-errors",
        action="store_true",
    ...
    extract = sub.add_parser(
        "extract", help="Scan + extract translation segments into a project JSON."
    )
    ...
    extract.add_argument(
        "--merge-from", help="Previous project JSON to merge targets/status/notes from."
    )
    extract.add_argument(
        "--json", action="store_true", help="Print project JSON to stdout."
    )
    ...
    build = sub.add_parser(
        "build",
        help="Apply translated targets from a project JSON and build an output folder/zip.",
    )
    build.add_argument(
        "project", help="Path to a project JSON with translated targets."
    )
    build.add_argument(
        "--out-dir", help="Output directory to write a built mod folder."
    )
    build.add_argument(
        "--out-zip", help="Output .zip path to write a built mod archive."
    )
    build.add_argument("--report", help="Write a Markdown diagnostics report.")
    build.add_argument(
        "--json",
        action="store_true",
        help="Print a machine-readable build summary JSON.",
    )
    build.add_argument(
        "--warnings-as-errors",
        action="store_true",
    ...
    report = sub.add_parser(
        "report", help="Export a Markdown diagnostics report from a project JSON."
    )
    report.add_argument("project", help="Path to a project JSON.")
    report.add_argument(
        "--source",
        choices=["project", "last-build"],
        default="project",
        help="Which diagnostics to render.",
    )
    report.add_argument(
        "--write",
        help="Write report to a file path (Markdown by default; JSON with --json).",
    )
    report.add_argument(
        "--json",
        action="store_true",
        help="Print a machine-readable JSON report (diagnostics + summary).",
    )
    report.add_argument(
        "--include-markdown",
        action="store_true",
        help="With --json, include a 'report_markdown' field.",
    )
    report.add_argument(
        "--print-stdout",
        action="store_true",
    ...
    val = sub.add_parser(
        "validate",
        help="Validate project segments (placeholders, glossary, and rule-based QA).",
    )
    val.add_argument("project", help="Path to a project JSON.")
    val.add_argument(
        "--json", action="store_true", help="Print machine-readable diagnostics JSON."
    )
    val.add_argument(
        "--update-project",
        action="store_true",
        help="Write validation diagnostics into project.diagnostics.",
    )
    val.add_argument(
        "--in-place",
        action="store_true",
        help="Overwrite the input project JSON (with --update-project).",
    )
    val.add_argument(
        "--write",
        help="Write updated project JSON to a file path (with --update-project).",
    )
    val.add_argument(
        "--warnings-as-errors",
        action="store_true",
    ...
    export_csv = sub.add_parser(
        "export-csv", help="Export project segments to a CSV for translation workflows."
    )
    ...
    glossary = sub.add_parser(
        "glossary", help="Import/export glossary CSV for a project."
    )
    ...
    glossary.add_argument(
        "--write", help="Write updated project JSON to a file path (import only)."
    )
    glossary.add_argument(
        "--in-place",
        action="store_true",
        help="Overwrite the input project JSON (import only).",
    )
    glossary.add_argument(
        "--no-overwrite",
        action="store_true",
        help="Do not overwrite existing glossary entry ids (import only).",
    )
    glossary.add_argument(
        "--json", action="store_true", help="Print a machine-readable summary JSON."
    )
    glossary.set_defaults(func=_cmd_glossary)

    import_csv = sub.add_parser(
        "import-csv", help="Import translations from a CSV back into a project JSON."
    )
    import_csv.add_argument("project", help="Path to a project JSON.")
    import_csv.add_argument(
        "--csv", required=True, help="CSV file path exported from export-csv."
    )
    import_csv.add_argument(
        "--write", help="Write updated project JSON to a file path."
    )
    import_csv.add_argument(
        "--in-place", action="store_true", help="Overwrite the input project JSON."
    )
    import_csv.add_argument(
        "--overwrite-target",
        action="store_true",
        help="Overwrite existing non-empty targets.",
    )
    import_csv.add_argument(
        "--no-status", action="store_true", help="Do not import status from CSV."
    )
    import_csv.add_argument(
        "--no-note", action="store_true", help="Do not import note from CSV."
    )
    import_csv.add_argument(
        "--json", action="store_true", help="Print a machine-readable summary JSON."
    )
    ...
    bulk = sub.add_parser(
        "bulk", help="Run bulk actions to speed up translation workflow."
    )
    bulk.add_argument("project", help="Path to a project JSON.")
    bulk.add_argument(
        "action", choices=["propagate", "apply-tm"], help="Bulk action to run."
    )
    bulk.add_argument(
        "--in-place", action="store_true", help="Overwrite the input project JSON."
    )
    ...
    bulk.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing non-empty targets."
    )
    bulk.add_argument(
        "--set-status",
        default="in_progress",
        help="Set status for updated segments (or empty to skip).",
    )
    bulk.add_argument("--min-score", default="95", help="TM min score (apply-tm only).")
    bulk.add_argument(
        "--query",
        default="",
        help="Scope bulk action to segments matching search query.",
    )
    bulk.add_argument(
        "--status",
        default="All",
        help="Scope bulk action by status: All|new|in_progress|reviewed.",
    )
    bulk.add_argument("--file", help="Scope bulk action to a specific file_path.")
    bulk.add_argument(
        "--sqlite-tm",
        action="store_true",
        help="Use SQLite TM at <project>.tm.sqlite3 (best-effort).",
    )
    ...
    sync = sub.add_parser(
        "sync", help="Filesystem-backed sync (simulated cloud): push/pull/status/merge."
    )
    sync.add_argument(
        "--root", required=True, help="Sync root folder (shared drive / cloud folder)."
    )
    ...
    sync.add_argument(
        "--in-place",
        action="store_true",
        help="Overwrite input project JSON when pushing.",
    )
    sync.add_argument(
        "--project-uid", dest="project_uid", help="Project UID (pull/merge)."
    )
    ...
    pred = sub.add_parser(
        "predict",
        help="Generate predictive target suggestions for a segment (local TM-based).",
    )
    ...
    tm = sub.add_parser(
        "tm",
        help="SQLite-backed translation memory (TM): migrate/import/export/suggest/concordance.",
    )
    tm.add_argument(
        "project",
        help="Path to a project JSON (used to pick default TM db and locales).",
    )
    tm.add_argument(
        "tm_action", choices=["migrate", "import", "export", "suggest", "concordance"]
    )
    tm.add_argument("--db", help="TM sqlite path (default: <project>.tm.sqlite3).")
    tm.add_argument(
        "--source-locale",
        help="Override project source locale (default: project.source_locale or und).",
    )
    tm.add_argument(
        "--target-locale",
        help="Override project target locale (default: project.target_locale or und).",
    )
    tm.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON (export only)."
    )
    tm.add_argument(
        "--format",
        choices=["json", "csv"],
        help="Import/export format (default: infer from extension).",
    )
    ...
    tm.add_argument(
        "--in-field", default="both", help="Concordance field: source|target|both."
    )
    ...
    ts4.add_argument(
        "--base-url",
        default="https://ts4rebels.cc/",
        help="Base URL (default: https://ts4rebels.cc/).",
    )
    ts4.add_argument(
        "--enable-network",
        action="store_true",
        help="Required to perform network requests.",
    )
    ts4.add_argument(
        "--allowed-hosts",
        default="ts4rebels.cc",
        help="Comma-separated allowlisted hosts for base-url (default: ts4rebels.cc).",
    )
    ts4.add_argument(
    ...
    ts4.add_argument(
        "--cookies",
        help="JSON string of session cookies for authentication.",
    )
    ts4.add_argument(
        "--allow-insecure-http",
        action="store_true",
    ...
    ts4login = ts4sub.add_parser(
        "login", help="Test login (reads env JPE_TS4REBELS_USER/PASS if args omitted)."
    )
    ts4login.add_argument(
        "--username", help="Username (prefer env var to avoid shell history)."
    )
    ts4login.add_argument(
        "--password", help="Password (prefer env var to avoid shell history)."
    )
    ts4login.set_defaults(func=_cmd_ts4rebels_login)

    ts4forum = ts4sub.add_parser(
        "forum", help="List topics from a forum id (best-effort HTML parsing)."
    )
    ...
    ts4topic = ts4sub.add_parser(
        "topic", help="Fetch a topic and extract links (best-effort HTML parsing)."
    )
    ...
    ts4dl = ts4sub.add_parser(
        "download-import",
        help="Download a .zip from a link and safely import it to a local folder.",
    )
    ts4dl.add_argument(
        "url", help="Direct download URL to a .zip (host must be allowlisted)."
    )
    ts4dl.add_argument(
        "--downloads-dir",
    ...
    ts4dl.add_argument(
        "--max-bytes", default=str(500_000_000), help="Max download size in bytes."
    )
    ts4dl.set_defaults(func=_cmd_ts4rebels_download_import)

    ts4preset = ts4sub.add_parser(
        "preset-validation",
        help="Apply TS4Rebels token validation presets to a project JSON.",
    )
    ts4preset.add_argument("--project", required=True, help="Path to a project JSON.")
    ts4preset.add_argument(
        "--in-place", action="store_true", help="Save changes back to --project."
    )
    ...
    ts4harvest.add_argument(
        "--max-files", default="35", help="Maximum number of unique files to copy."
    )
    ts4harvest.add_argument(
    ...
    ts4an = ts4sub.add_parser(
        "analyze-samples",
        help="Analyze a local TS4Rebels sample folder for tokens/placeholders.",
    )
    ts4an.add_argument(
        "path",
        nargs="?",
    ...
    ts4imp = sub.add_parser(
        "ts4rebels-import",
        help="Import a TS4Rebels translation pack (CSV/JSON) into a project.",
    )
    ts4imp.add_argument("--project", required=True, help="Path to project JSON.")
    ts4imp.add_argument(
        "--in", dest="input", required=True, help="Translation pack path (.csv/.json)."
    )
    ts4imp.add_argument(
        "--overwrite-targets",
        action="store_true",
        help="Overwrite non-empty targets (default: off).",
    )
    ts4imp.add_argument(
        "--import-glossary",
        action="store_true",
        help="Import glossary terms if present in the pack.",
    )
    ts4imp.add_argument(
        "--in-place", action="store_true", help="Save changes back to --project."
    )
    ...
    diag = sub.add_parser(
        "diagnose",
        help="Analyze Sims 4 game crash logs (BetterExceptions integration).",
    )
    diag.add_argument(
        "--logs", nargs="+", help="Explicit path to log files (LastException*.txt)."
    )
    diag.add_argument("--limit", default="5", help="Max number of logs to analyze.")
    diag.add_argument(
        "--json", action="store_true", help="Output machine-readable JSON."
    )
    diag.add_argument("--write", help="Write report to a file path.")
    diag.set_defaults(func=_cmd_diagnose)

    ignite = sub.add_parser(
        "ignite",
        help="Industrial Ignition: Automate JPE-Live mod setup and environment check.",
    )
    ignite.add_argument(
        "--mods-path", help="Override Sims 4 Mods folder path (auto-detected if omitted)."
    )
    ignite.add_argument(
