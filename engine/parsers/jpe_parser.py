...
class JpeParser:
    ...
    def parse_project(self, root: Path) -> tuple[ProjectIR, List[EngineError]]:
        ...
        project_metadata = ProjectMetadata(
            name=root.name, project_id=root.name, version="0.0.0"
        )
    ...
    def _parse_file(
        self, content: str, file_path: Path, project_ir: ProjectIR
    ) -> List[EngineError]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if kv_match:
                ...
                if current_section == "project":
                    ...
                    current_element = self._parse_interaction_line(
                        key, value, current_element
                    )
                    # If we have a complete interaction, add it to the project
                    if key == "end" and current_element is not None:
                        try:
                            interaction = self._create_interaction_from_dict(
                                current_element
                            )
                    ...
                    current_element = self._parse_trait_line(
                        key, value, current_element
                    )
                    if key == "end" and current_element is not None:
                        try:
                    ...
                    current_element = self._parse_string_line(
                        key, value, current_element
                    )
                    if key == "end" and current_element is not None:
                        try:
    ...
    def _parse_project_metadata(
        self, key: str, value: str, project_ir: ProjectIR
    ) -> None:
    ...
    def _parse_interaction_line(
        self, key: str, value: str, current_element: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
