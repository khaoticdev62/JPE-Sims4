...
class JpeXmlForkParser:
    ...
    def parse_files(self, paths: Iterable[Path]) -> tuple[ProjectIR, List[EngineError]]:
        ...
        for file_path in paths:
            try:
                ...
                if (
                    root.tag != "jpe-project"
                    and not root.get("format") == "jpe-xml-fork"
                ):
    ...
    def _parse_project_element_fork(
        self,
        element: ET.Element,
    ...
    def _parse_project_metadata_fork(
        self, element: ET.Element, project_ir: ProjectIR
    ) -> None:
    ...
    def _parse_interaction_fork(self, element: ET.Element) -> Interaction:
        ...
        return Interaction(
            id=interaction_id,
            display_name_key=display_name_elem.text
            if display_name_elem is not None and display_name_elem.text
            else None,
            description_key=description_elem.text
            if description_elem is not None and description_elem.text
            else None,
            participants=participants,
            autonomy_disabled=autonomy_disabled_elem.text.lower() == "true"
            if autonomy_disabled_elem is not None and autonomy_disabled_elem.text
            else False,
            tests=conditions,
            loot_actions=[],  # Would require more complex parsing
        )

    def _parse_buffs_fork(
        self,
        element: ET.Element,
        project_ir: ProjectIR,
        file_path: Path,
    ...
    def _parse_enum_fork(self, element: ET.Element) -> EnumDefinition:
        ...
        if options_elem is not None:
            for option_elem in options_elem.findall("option"):
                ...
                options.append(
                    EnumOption(name=name_attr if name_attr else "unnamed", value=value)
                )
        ...
        return EnumDefinition(id=enum_id, options=options)

    def _parse_strings_fork(
        self,
        element: ET.Element,
        project_ir: ProjectIR,
        file_path: Path,
    ...
    def _parse_documentation_fork(
        self, element: ET.Element, file_path: Path, errors: List[EngineError]
    ) -> None:
