...
import defusedxml.ElementTree as ET  # SECURITY FIX: Use defusedxml to prevent XXE attacks
...
class XmlParser:
    ...
    def parse_files(self, paths: Iterable[Path]) -> tuple[ProjectIR, List[EngineError]]:
        ...
        project_metadata = ProjectMetadata(
            name="default_project", project_id="default_project", version="0.0.0"
        )
        ...
        for file_path in paths:
            try:
                ...
                if root.tag != 'I c="Tuning"' and not any(
                    elem.tag == "Tunings" for elem in root
                ):
    ...
    def _parse_element_recursive(
        ...
        try:
            ...
            for child in element:
                self._parse_element_recursive(
                    child, project_ir, file_path, errors, element_id, element_type
                )
    ...
    def _parse_interaction_element(
        self, element: ET.Element, element_id: str
        ...
        try:
            ...
            autonomy_disabled = self._get_bool_from_element(
                element, "autonomy_disabled", False
            )
    ...
    def _parse_interaction_participants(
        ...
        for path in possible_participant_paths:
            for participant_elem in element.findall(path):
                ...
                role = (
                    participant_elem.get("role")
                    or self._get_value_from_element(participant_elem, "role")
                    or "unknown"
                )
                description = self._get_value_from_element(
                    participant_elem, "description"
                )

                participants.append(
                    InteractionParticipant(role=role, description=description)
                )

        return participants

    def _parse_buff_element(
        self, element: ET.Element, element_id: str
    ) -> Optional[Buff]:
        ...
        try:
            ...
            if duration_elem is not None and duration_elem.text:
                try:
                    ...
                    )  # Duration might be float in XML
                except ValueError:
                    pass
    ...
    def _parse_associated_buffs(self, element: ET.Element) -> List[ResourceId]:
        ...
        for buff_elem in buff_elements:
            buff_name = (
                buff_elem.text
                or buff_elem.get("value")
                or buff_elem.get("name")
                or "unknown"
            )
    ...
    def _parse_enum_element(
        ...
        try:
            ...
            option_elements = element.findall(".//option") + element.findall(
                ".//options/*"
            )

            for i, option_elem in enumerate(option_elements):
                option_name = (
                    option_elem.get("name") or option_elem.get("key") or f"option_{i}"
                )
            ...
            return EnumDefinition(id=ResourceId(name=element_id), options=options)
        except Exception:
            # If parsing fails, return a minimal enum with no options
            return EnumDefinition(id=ResourceId(name=element_id), options=[])

    def _get_value_from_element(
        self, element: ET.Element, tag_name: str
    ) -> Optional[str]:
    ...
    def _get_bool_from_element(
        self, element: ET.Element, tag_name: str, default: bool
    ) -> bool:
