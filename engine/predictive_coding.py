...
import json  # SECURITY FIX: Replaced pickle with JSON to prevent RCE
...
class PredictiveCodingModel:
    ...
    def __init__(self, model_path: Optional[Path] = None):
        ...
        self.pattern_db: Dict[str, Counter] = defaultdict(
            Counter
        )  # Token -> Next token frequencies
        self.error_patterns: Dict[str, List[str]] = defaultdict(
            list
        )  # Common error patterns and fixes
        self.syntax_templates: Dict[str, str] = {}  # Common syntax templates
        self.user_behavior_db: Dict[str, Counter] = defaultdict(
            Counter
        ...
        try:
            ...
            self.sentinel_logger.log_exception(
                ...
                context_info={
                    ...
                    "project_id": project_ir.metadata.project_id
        ...
        if project_ir.metadata:
            tokens.extend(
                [
                    "define",
                    "project",
                    project_ir.metadata.name or "",
                    project_ir.metadata.author or "",
                    project_ir.metadata.version or "",
                ]
            )
        ...
        for interaction in project_ir.interactions:
            tokens.extend(
                [
                    "define",
                    "interaction",
                    interaction.resource_id.name if interaction.resource_id else "",
                    "name:",
                    "display_name:",
                    "description:",
                    "class:",
        ...
        for buff in project_ir.buffs:
            tokens.extend(
                [
                    "define",
                    "buff",
                    buff.resource_id.name if buff.resource_id else "",
                    "name:",
                    "display_name:",
                    "description:",
                    "class:",
        ...
        for trait in project_ir.traits:
            tokens.extend(
                [
                    "define",
                    "trait",
                    trait.resource_id.name if trait.resource_id else "",
                    "name:",
                    "display_name:",
                    "description:",
                    "class:",
        ...
        for enum in project_ir.enums:
            tokens.extend(
                [
                    "define",
                    "enum",
                    enum.resource_id.name if enum.resource_id else "",
                    "name:",
                    "description:",
                    "options:",
                ]
    ...
    def predict_error_recovery(
        self, error: EngineError, context: PredictionContext
    ) -> List[Prediction]:
    ...
    def _suggest_syntax_fixes(
        ...
        if (
            "end" not in context.tokens_after_cursor
            and "define" in context.tokens_before_cursor
        ):
    ...
    def suggest_template_completion(
    ...
    def save_model(self, path: Path):
        """Save the trained model to disk using JSON (secure serialization)."""
        data = {
            "pattern_db": {k: dict(v) for k, v in self.pattern_db.items()},
            "error_patterns": dict(self.error_patterns),
            "syntax_templates": self.syntax_templates,
            "user_behavior_db": {k: dict(v) for k, v in self.user_behavior_db.items()},
        }

        # Create directory if it doesn't exist
        path.parent.mkdir(parents=True, exist_ok=True)

        # SECURITY FIX: Use JSON instead of pickle to prevent RCE
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)

    def load_model(self, path: Path):
        """Load a trained model from disk using JSON (secure deserialization)."""
        try:
            # SECURITY FIX: Use JSON instead of pickle to prevent RCE
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)

                # Convert dicts back to appropriate types
                self.pattern_db = defaultdict(
                    Counter,
                    {k: Counter(v) for k, v in data.get("pattern_db", {}).items()},
                )
                self.error_patterns = defaultdict(list, data.get("error_patterns", {}))
                self.syntax_templates = data.get("syntax_templates", {})
                self.user_behavior_db = defaultdict(
                    Counter,
                    {
                        k: Counter(v)
                        for k, v in data.get("user_behavior_db", {}).items()
                    },
                )
        except Exception as e:
            self.sentinel_logger.log_exception(
                e, context_info={"model_path": str(path)}
            )
...
class PredictiveCodingSystem:
    ...
    def get_completion_suggestions(self, context: PredictionContext) -> List[str]:
        ...
        return [
            ...
            in [PredictionType.SYNTAX_COMPLETION, PredictionType.REFERENCE_SUGGESTION]
        ]
    ...
    def get_error_suggestions(
        ...
        return [
            ...
            and pred.prediction_type == PredictionType.ERROR_RECOVERY
        ]
