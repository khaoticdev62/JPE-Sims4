...
import {
  X, RefreshCw, Monitor,
  Star, Smile, Briefcase, Users, Zap, Activity} from "lucide-react";
...
function InteractionPieFrame({ sliders: _sliders }: { sliders: Slider[] }) {
  const options = [
...
export function LivePreview({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const FrameComponent = {
    ...
    "aspiration":      AspirationFrame}[activeFrame];
      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center"
          ...
          <motion.div initial={{opacity:0,scale:0.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.97,y:12}}
            ...
            style={{width:"min(920px,97vw)",height:"min(660px,90vh)",background:T.bgElevated,border:`1px solid ${T.border}`,boxShadow:`${T.shadow2Xl},inset 0 1px 0 rgba(255,255,255,0.04)`}}>
