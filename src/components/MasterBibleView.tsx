...
/* ═══ FILE TREE ITEM ═══ */
function FileTreeItem({ node, depth = 0, onSelect, selectedName }: { node: TreeNode; depth?: number; onSelect?: (name: string) => void; selectedName?: string | null }) {
  const [expanded, setExpanded] = useState(node.name === "Project" || node.name === "MASTER BIBLE");
  const isFolder = node.type === "folder";
  const isSelected = !isFolder && selectedName === node.name;
  ...
  return (
    <div>
      ...
      {isFolder && expanded && node.children?.map((child, i) => <FileTreeItem key={`${child.name}-${i}`} node={child} depth={depth + 1} onSelect={onSelect} selectedName={selectedName} />)}
    </div>
  );
}
...
export function MasterBibleView() {
  const { currentProject, getFile: _getFile } = useProjectStore();
  const { diagnostics } = useDiagnosticStore();
  const { usageStats: _usageStats, activeProvider: _activeProvider } = useAIStore();
  ...
  // Memoized dynamic file tree
  const dynamicFileTree = useMemo(() => {
    if (!currentProject) return [];
    
    const root: TreeNode = { name: currentProject.name, type: "folder", children: [] };
    const files = currentProject.files.filter(f => f.name.toLowerCase().includes(fileFilter.toLowerCase()));
    ...
    files.forEach(file => {
      ...
      parts.forEach((part, idx) => {
        ...
        if (!existing) {
          ...
          const fileDiagnostics = diagnostics.filter(d => d.fileId === file.id);
          const status = fileDiagnostics.length > 0 
            ? (fileDiagnostics.some(d => d.severity === 'error') ? 'pending' : 'review') 
            : 'done';
    ...
    return [root];
  }, [currentProject, fileFilter, diagnostics]);

  const activeConflicts = useMemo(() => {
    if (!selectedFileId) return diagnostics;
    return diagnostics.filter(d => d.fileId === selectedFileId);
  }, [diagnostics, selectedFileId]);

  const _resolvedConflicts = useMemo(() => {
    // For now, mock resolved as empty since we don't track resolution history yet
    return [];
  }, []);
      ...
      <div className="flex items-center justify-between px-4 h-9 flex-shrink-0" style={{ background: T.bgPanel, borderBottom: `1px solid ${T.border}` }}>
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${T.borderSubtle}` }}>
            ...
            <span style={{ fontSize: 9, color: T.textSecondary }}>{currentProject?.name || "Project"} / English (US)</span>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex flex-col flex-shrink-0" style={{ width: fileTreeW, borderRight: `1px solid ${T.border}`, background: T.bgPanel }}>
          ...
          <div className="flex-1 overflow-y-auto py-0.5">
            {dynamicFileTree.map((node, i) => (
              <FileTreeItem 
                key={`root-${i}`} 
                node={node} 
                selectedName={selectedFileId}
                onSelect={(name) => {
                  const file = currentProject?.files.find(f => f.name === name);
                  setSelectedFileId(file ? file.id : name);
                }} 
              />
            ))}
          </div>
          ...
          <div className="px-3 py-1.5 flex items-center justify-between" style={{ borderTop: `1px solid ${T.border}` }}>
            <span style={{ fontSize: 7, fontFamily: T.mono, color: T.textMuted }}>{currentProject?.files.length || 0} files</span>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1"><div className="w-1 h-1 rounded-full" style={{ background: T.emerald }} /><span style={{ fontSize: 7, fontFamily: T.mono, color: T.textMuted }}>{currentProject?.files.length || 0}</span></div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-3 pt-3 pb-2">
              ...
              <div>
                ...
                {activeConflicts.map((log: any, i: number) => (
                  <div key={`act-${i}`} className="flex items-start gap-2 px-2 py-1.5 rounded mb-0.5 transition-colors"
                    ...
                    <span style={{ fontSize: 8, fontFamily: T.mono, color: T.rose, fontWeight: 600, flexShrink: 0 }}>{log.id.substring(0, 4)}</span>
                    <span style={{ fontSize: 9, color: T.textSecondary, lineHeight: 1.4, flex: 1 }}>{log.message}</span>
