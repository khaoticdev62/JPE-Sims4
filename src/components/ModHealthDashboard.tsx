...
import {
  X, Shield, CheckCircle2, AlertTriangle,
  TrendingUp, Zap, ArrowRight, RefreshCw, CloudLightning,
} from "lucide-react";
import {
...
function ScoreGauge({ score }: { score: number }) {
  ...
  const _x1 = cx + r*Math.cos(toRad(0));
  const _y1 = cy + r*Math.sin(toRad(0));
...
export function ModHealthDashboard({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const { isConnected, metrics, tuningExecCount } = useLiveSyncStore();

  // Story 4.1: Industrial Telemetry Lifecycle
  useEffect(() => {
    telemetry.startPolling();
    return () => telemetry.stopPolling();
  }, []);
  ...
  const handleDeploySync = async () => {
    ...
    setRefreshing(false);
    toast.success("Industrial Link Re-established", {
      description: "Handshake with Sims 4 engine completed."
    });
  };

  useEffect(() => {
    if (isOpen && !isConnected) {
      // Auto-connect (or simulate) when dashboard opens
      liveService.connect();
    }
  }, [isOpen, isConnected]);
      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center"
          ...
          <motion.div initial={{opacity:0,scale:0.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.97,y:12}}
            ...
            style={{width:"min(1040px,97vw)",height:"min(720px,92vh)",background:T.bgElevated,border:`1px solid ${T.border}`,boxShadow:`${T.shadow2Xl},inset 0 1px 0 rgba(255,255,255,0.04)`}}>
            ...
            <div className="flex items-center justify-between px-6 py-3 flex-shrink-0" style={{borderBottom:`1px solid ${T.border}`}}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div style={{fontSize:10,color:isConnected ? T.emerald : T.textMuted,fontFamily:T.mono}}>
                    {isConnected ? "LIVE ENGINE LINK ACTIVE" : "ENGINE LINK DISCONNECTED"} · {tuningExecCount} blocks monitored
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => sims4.deploySpectralBridge()}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all hover:brightness-110"
                  style={{fontSize:11,fontWeight:700,color:T.textPrimary,background:`linear-gradient(135deg,${T.violet},${T.cyan})`,border:`1px solid rgba(255,255,255,0.1)`}}>
                  <CloudLightning size={11}/> Deploy Sync Bridge
                </button>
                <button onClick={handleDeploySync} disabled={refreshing}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
              ...
              {/* Story 4.2: Quantum Mission Control */}
              <div className="mb-8">
                <TelemetryDashboard />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="flex flex-col items-center justify-center p-6 rounded-2xl" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  ...
                  <ScoreGauge score={isConnected ? 94 : 0}/>
                  <div style={{fontSize:10,color:T.textMuted,marginTop:8,textAlign:"center"}}>
                    {isConnected ? "Engine synchronizing logic blocks" : "Connect to engine for live diagnostics"}
                  </div>
                </div>

                {/* Radar chart (Modified for live latency/perf) */}
                <div className="rounded-2xl overflow-hidden p-4" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  <div style={{fontSize:9,fontWeight:700,color:T.textMuted,letterSpacing:"0.07em",marginBottom:8}}>ENGINE TELEMETRY</div>
                  <div className="flex flex-col gap-4 py-4 px-2">
                    <div className="flex justify-between items-center">
                      <div style={{fontSize:10,color:T.textMuted}}>CPU_LOAD</div>
                      <div style={{fontSize:12,fontWeight:700,color:T.cyan,fontFamily:T.mono}}>{metrics.cpu}%</div>
                    </div>
                    <div className="flex justify-between items-center">
                      <div style={{fontSize:10,color:T.textMuted}}>ID_LATENCY</div>
                      <div style={{fontSize:12,fontWeight:700,color:T.violet,fontFamily:T.mono}}>{metrics.latency}ms</div>
                    </div>
                    <div className="flex justify-between items-center">
                      <div style={{fontSize:10,color:T.textMuted}}>CORE_READY</div>
                      <div style={{fontSize:12,fontWeight:700,color:T.emerald,fontFamily:T.mono}}>100%</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{height:100}}>
                  <SafeChartContainer className="w-full h-full">
                    <BarChart data={BAR_DATA} margin={{top:4,right:8,bottom:4,left:8}} barCategoryGap="30%">
                      ...
                      <Tooltip content={<CustomTooltip/>}/>
                      <Bar dataKey="score" radius={[4,4,0,0]} minPointSize={0}>
