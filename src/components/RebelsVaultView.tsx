...
const V = {
  ...
  bgGlassHeavy: "rgba(10, 12, 16, 0.92)",
  border: "rgba(255, 255, 255, 0.06)",
  borderSubtle: "rgba(255, 255, 255, 0.03)",
  borderCyan: "rgba(99, 179, 237, 0.2)",
  borderViolet: "rgba(139, 92, 246, 0.25)",
  borderEmerald: "rgba(72, 187, 120, 0.2)",
  borderRose: "rgba(252, 129, 129, 0.2)",
  cyan: "#63B3ED",
  cyanBright: "#90CDF4",
  cyanDim: "rgba(99, 179, 237, 0.06)",
  cyanMid: "rgba(99, 179, 237, 0.12)",
  violet: "#8B5CF6",
  violetBright: "#A78BFA",
  ...
  emeraldDim: "rgba(72, 187, 120, 0.06)",
  rose: "#FC8181",
  roseBright: "#FB7185",
  roseDim: "rgba(252, 129, 129, 0.06)",
  amber: "#F6AD55",
  amberBright: "#FBBF24",
  amberDim: "rgba(246, 173, 85, 0.06)",
  textPrimary: "#E2E8F0",
...
function GlassPanel({ children, className = "", style = {}, glow, borderColor }: {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: V.noiseSvg, opacity: 0.6 } as any} />
...
function Eyebrow({ children, color = V.textTertiary }: { children: React.ReactNode; color?: string }) {
  return (
    <span className="uppercase select-none tracking-[0.14em]" style={{ fontFamily: V.sans, fontSize: 10, fontWeight: 700, color } as any}>
      {children}
    </span>
  );
}
...
function NeuralDot({ color = V.violet, size = 6 }: { color?: string; size?: number }) {
  return (
    <div className="relative flex items-center justify-center" style={{ width: size + 4, height: size + 4 } as any}>
      <div className="rounded-full animate-pulse" style={{ width: size, height: size, background: color, boxShadow: `0 0 10px ${color}80, 0 0 4px ${color}` } as any} />
    </div>
  );
}
...
function SeverityDot({ severity }: { severity: DiagNode["severity"] }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 10, height: 10 } as any}>
      <div className="rounded-full" style={{ width: 7, height: 7, background: cfg.color, boxShadow: `0 0 8px ${cfg.color}70` } as any} />
      {(severity === "critical") && (
        <div className="absolute inset-0 rounded-full animate-ping" style={{ background:  cfg.color, opacity: 0.2 } as any} />
      )}
    </div>
  );
}
...
function ConflictScoreBar({ score, height = 8 }: { score: number; height?: number }) {
  ...
  return (
    <div className="w-full relative">
      <div className="w-full rounded-full overflow-hidden" style={{ height, background: "rgba(255,255,255,0.04)" } as any}>
        <div
      ...
      {/* Tick marks */}
      <div className="absolute top-0 left-0 right-0 flex justify-between px-0" style={{ height } as any}>
        {[25, 50, 75].map(tick => (
          <div key={tick} className="absolute top-0" style={{ left: `${tick}%`, width: 1, height, background: "rgba(255,255,255,0.06)" } as any} />
        ))}
      </div>
    </div>
  );
    ...
    >
      <div className="flex items-start justify-between p-4">
        <div className="flex flex-col gap-1.5">
          ...
          <div style={{ fontFamily: V.mono, fontSize: 26, fontWeight: 800, color: V.textPrimary, lineHeight: 1 } as any}>
            {value}
          </div>
          {sub && (
            <div className="flex items-center gap-1.5 mt-0.5">
              <NeuralDot color={accentColor} size={4} />
              <span style={{ fontSize: 10, fontFamily: V.mono, color: accentColor, fontWeight: 600 } as any}>{sub}</span>
            </div>
          )}
        </div>
        <div className="p-2 rounded-xl" style={{ background:  `${accentColor}08`, border: `1px solid ${accentColor}15` } as any}>
          <Icon size={18} color={accentColor} />
        </div>
      </div>
      {/* Bottom accent line */}
      <div className="h-px mt-auto" style={{ background:  `linear-gradient(90deg, transparent, ${accentColor}25, transparent)` } as any} />
    </GlassPanel>
  );
}
...
function TriageContent({ filteredNodes, selectedNode, setSelectedNode, searchQuery, setSearchQuery }: {
  ...
  return (
    <>
      <GlassPanel className="flex flex-col" style={{ width: panelW, flexShrink: 0 } as any} glow={`${V.violet}04`}>
        <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom: `1px solid ${V.border}` } as any}>
          <div className="flex items-center gap-2">
            <Box size={13} color={V.violet} />
            <Eyebrow color={V.textSecondary}>Diagnostic Nodes</Eyebrow>
            <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>({filteredNodes.length})</span>
          </div>
          <MoreHorizontal size={13} color={V.textMuted} className="cursor-pointer" />
        </div>
        <div className="px-3 py-2 flex-shrink-0" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg" style={{ background:  "rgba(255,255,255,0.03)", border: `1px solid ${V.border}` } as any}>
            ...
            <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search nodes..."
              className="flex-1 bg-transparent outline-none" style={{ fontSize: 11, fontFamily: V.sans, color: V.textPrimary } as any} />
          </div>
        </div>
          ...
          {filteredNodes.map((node, i) => {
            ...
            return (
              ...
              <div className="relative flex items-start gap-3 px-4 py-3 cursor-pointer transition-all"
                style={{ background:  isActive ? V.violetDim : "transparent", borderLeft: isActive ? `2px solid ${V.violet}` : "2px solid transparent" } as any}
                ...
                <div className="flex flex-col items-center gap-1 pt-0.5 flex-shrink-0">
                  ...
                  {i < filteredNodes.length - 1 && <div className="w-px flex-1 min-h-[16px]" style={{ background:  `linear-gradient(to bottom, ${svCfg.color}20, transparent)` } as any} />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: V.textMuted } as any}>{node.id}</span>
                    <span className="px-1.5 py-0.5 rounded" style={{ fontSize: 8, fontFamily: V.mono, fontWeight: 700, color: svCfg.color, background: `${svCfg.color}10`, border: `1px solid ${svCfg.color}15` } as any}>{svCfg.label}</span>
                  </div>
                  <div className="mt-1" style={{ fontSize: 12, fontWeight: 600, color: V.textPrimary, lineHeight: 1.3 } as any}>{node.name}</div>
                  <div className="flex items-center gap-2 mt-1.5">
                    <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textTertiary } as any}>{node.module}</span>
                    <span style={{ fontSize: 8, color: V.textDim } as any}>•</span>
                    <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>{node.lastScan}</span>
                  </div>
        ...
        <div className="flex items-center justify-between px-4 py-2 flex-shrink-0" style={{ borderTop: `1px solid ${V.border}` } as any}>
          <span style={{ fontSize: 9, color: V.textMuted } as any}>Last full scan: 2m ago</span>
          <NeuralDot color={V.violet} size={4} />
        </div>
      </GlassPanel>
      <GlassPanel className="flex-1 flex flex-col" glow={`${V.cyan}04`} borderColor={V.border}>
        <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ borderRadius: 14 } as any}><NetworkCanvas /></div>
        <div className="flex items-center justify-between px-5 py-3 flex-shrink-0" style={{ borderBottom: `1px solid ${V.border}`, background: "rgba(3,3,5,0.6)" } as any}>
          <div className="flex items-center gap-3">
            <Shield size={15} color={V.cyan} />
            <span style={{ fontSize: 15, fontWeight: 800, color: V.textPrimary, fontFamily: V.display } as any}>Issue Diagnosis</span>
            <span className="px-2 py-0.5 rounded-lg" style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: V.violetBright, background: V.violetDim, border: `1px solid ${V.borderViolet}` } as any}>{activeNode?.id}</span>
          </div>
          <div className="flex items-center gap-2">
            <NeuralDot color={V.violet} size={4} />
            <span style={{ fontSize: 9, fontFamily: V.mono, color: V.textTertiary } as any}>AI ANALYZING</span>
          </div>
        </div>
        <AnimatePresence mode="wait">
          {activeNode && (
            <motion.div key={activeNode.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
              ...
              <div className="flex items-center gap-6 px-5 py-3" style={{ borderBottom: `1px solid ${V.borderSubtle}`, background: "rgba(3,3,5,0.4)" } as any}>
                {[{ label: "MOD NAME", value: activeNode.name, color: V.textPrimary, weight: 700 }, { label: "MODULE", value: activeNode.module, color: V.cyan, weight: 600 }].map((f, fi) => (
                  <div key={fi} className="flex flex-col gap-0.5">
                    <span style={{ fontSize: 9, color: V.textTertiary, letterSpacing: "0.1em" } as any}>{f.label}</span>
                    <span style={{ fontFamily: V.mono, fontSize: 13, fontWeight: f.weight, color: f.color } as any}>{f.value}</span>
                  </div>
                ))}
                <div className="flex flex-col gap-0.5">
                  <span style={{ fontSize: 9, color: V.textTertiary, letterSpacing: "0.1em" } as any}>SEVERITY</span>
                  <div className="flex items-center gap-1.5">
                    <SeverityDot severity={activeNode.severity} />
                    <span style={{ fontFamily: V.mono, fontSize: 13, fontWeight: 700, color: severityConfig[activeNode.severity].color } as any}>{severityConfig[activeNode.severity].label}</span>
                  </div>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span style={{ fontSize: 9, color: V.textTertiary, letterSpacing: "0.1em" } as any}>STATUS</span>
                  <span style={{ fontFamily: V.mono, fontSize: 13, fontWeight: 600, color: activeNode.status === "active" ? V.amber : activeNode.status === "resolved" ? V.emerald : V.cyan } as any}>{activeNode.status.toUpperCase()}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span style={{ fontSize: 9, color: V.textTertiary, letterSpacing: "0.1em" } as any}>ISSUES</span>
                  <span style={{ fontFamily: V.mono, fontSize: 13, fontWeight: 700, color: V.textPrimary } as any}>{activeNode.issueCount}</span>
                </div>
              </div>
              <div className="px-5 py-4" style={{ borderBottom: `1px solid ${V.borderSubtle}`, background: "rgba(3,3,5,0.35)" } as any}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span style={{ fontFamily: V.mono, fontSize: 24, fontWeight: 900, color: V.textPrimary } as any}>{activeNode.conflictScore}%</span>
                </div>
                <ConflictScoreBar score={activeNode.conflictScore} height={12} />
                <div className="flex items-center justify-between mt-2">
                  <span style={{ fontSize: 9, fontFamily: V.mono, color: V.emerald } as any}>0% Safe</span>
                  <span style={{ fontSize: 9, fontFamily: V.mono, color: V.amber } as any}>50% Elevated</span>
                  <span style={{ fontSize: 9, fontFamily: V.mono, color: V.rose } as any}>100% Critical</span>
                </div>
              </div>
              <div className="px-5 py-4" style={{ borderBottom: `1px solid ${V.borderSubtle}`, background: "rgba(3,3,5,0.3)" } as any}>
                <Eyebrow color={V.textTertiary}>Analysis Report</Eyebrow>
                <p className="mt-2" style={{ fontSize: 12, color: V.textSecondary, lineHeight: 1.7 } as any}>{activeNode.description}</p>
              </div>
              <div className="flex gap-3 px-5 py-4" style={{ background:  "rgba(3,3,5,0.25)" } as any}>
                <GlassPanel className="flex-1" style={{ borderRadius: 10 } as any}>
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: `1px solid ${V.border}` } as any}>
                  ...
                  <div className="p-3 space-y-1.5">
                    {activeNode.affectedFiles.map((file, i) => (
                      <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-md" style={{ background:  "rgba(255,255,255,0.02)", border: `1px solid ${V.borderSubtle}` } as any}>
                        <FileCode size={10} color={V.textMuted} />
                        <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textSecondary } as any}>{file}</span>
                      </div>
                    ))}
                  </div>
                </GlassPanel>
                <GlassPanel className="flex-1" style={{ borderRadius: 10 } as any} glow={`${V.emerald}06`} borderColor={V.borderEmerald}>
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: `1px solid ${V.border}` } as any}>
                    <Sparkles size={11} color={V.emerald} /><Eyebrow color={V.emerald}>AI Suggested Fix</Eyebrow><NeuralDot color={V.violet} size={4} />
                  </div>
                  <div className="p-3">
                    <p style={{ fontSize: 11, color: V.textSecondary, lineHeight: 1.7 } as any}>{activeNode.suggestedFix}</p>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                      className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg w-full justify-center"
                      style={{ background:  `linear-gradient(135deg, ${V.emerald}20, ${V.cyan}15)`, border: `1px solid ${V.borderEmerald}`, boxShadow: `0 0 12px ${V.emerald}10`, cursor: "pointer" } as any}>
                      <Zap size={12} color={V.emeraldBright} />
                      <span style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: V.emeraldBright, letterSpacing: "0.08em" } as any}>APPLY FIX</span>
                    </motion.button>
                  </div>
                </GlassPanel>
              </div>
              <div className="flex items-center gap-2 px-5 py-3" style={{ borderTop: `1px solid ${V.border}`, background: "rgba(3,3,5,0.4)" } as any}>
                {[
                  ...
                  <motion.button key={bi} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                    ...
                    style={{ background:  btn.bg, border: `1px solid ${btn.border}`, cursor: "pointer" } as any}>
                    <btn.icon size={12} color={btn.color} />
                    <span style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: btn.color, letterSpacing: "0.06em" } as any}>{btn.label}</span>
                  </motion.button>
                ))}
                <div className="flex-1" />
                <div className="flex items-center gap-2">
                  <NeuralDot color={V.violet} size={4} />
                  <span style={{ fontSize: 9, fontFamily: V.mono, color: V.textMuted } as any}>Neural Engine Processing</span>
                </div>
              </div>
            </motion.div>
          )}
...
function SystemMetricRow({ label, value, pct, color }: { label: string; value: string; pct: number; color: string }) {
  return (
    <div className="flex items-center gap-3 py-2.5 px-1" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
      <span className="w-28" style={{ fontSize: 11, fontWeight: 600, color: V.textSecondary } as any}>{label}</span>
      <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background:  "rgba(255,255,255,0.04)" } as any}>
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
          className="h-full rounded-full" style={{ background:  `linear-gradient(90deg, ${color}, ${color}80)`, boxShadow: `0 0 8px ${color}30` } as any} />
      </div>
      <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: 700, color, minWidth: 60, textAlign: "right" as const } as any}>{value}</span>
    </div>
  );
}
...
function SystemContent() {
  ...
  return (
    <>
      <GlassPanel className="flex flex-col" style={{ width: panelW, flexShrink: 0 } as any} glow={`${V.cyan}04`}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
        ...
        <div className="flex items-center justify-between px-4 py-2" style={{ borderTop: `1px solid ${V.border}` } as any}>
          <span style={{ fontSize: 9, fontFamily: V.mono, color: V.textMuted } as any}>Uptime: 14d 6h 42m</span>
          <NeuralDot color={V.emerald} size={4} />
        </div>
      </GlassPanel>
      <GlassPanel className="flex-1 flex flex-col" glow={`${V.violet}04`}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
          <div className="flex items-center gap-2">
            <Server size={13} color={V.violet} />
            <Eyebrow color={V.textSecondary}>Active Processes</Eyebrow>
            <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>({systemProcesses.length})</span>
          </div>
          <div className="flex items-center gap-2">
            <NeuralDot color={V.emerald} size={4} />
            <span style={{ fontSize: 9, fontFamily: V.mono, color: V.textTertiary } as any}>ALL NOMINAL</span>
          </div>
        </div>
        <div className="flex items-center gap-4 px-5 py-2" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
          {["PROCESS", "PID", "CPU", "MEMORY", "STATUS"].map(h => (
            <span key={h} className={h === "PROCESS" ? "flex-1" : ""} style={{ fontSize: 8, fontWeight: 700, color: V.textMuted, letterSpacing: "0.12em", minWidth: h === "PROCESS" ? undefined : 70 } as any}>{h}</span>
          ))}
        </div>
          ...
          {systemProcesses.map(proc => (
            <StaggerItem key={proc.pid}>
              <div className="flex items-center gap-4 px-5 py-3 transition-colors cursor-pointer"
                style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}
                onMouseEnter={e => { e.currentTarget.style.background = V.bgHover; }}
                onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                <div className="flex-1 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background:  proc.color, boxShadow: `0 0 4px ${proc.color}60` } as any} />
                  <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: 600, color: V.textPrimary } as any}>{proc.name}</span>
                </div>
                <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textMuted, minWidth: 70 } as any}>{proc.pid}</span>
                <span style={{ fontFamily: V.mono, fontSize: 10, color: V.cyan, fontWeight: 600, minWidth: 70 } as any}>{proc.cpu}</span>
                <span style={{ fontFamily: V.mono, fontSize: 10, color: V.violetBright, minWidth: 70 } as any}>{proc.mem}</span>
...
function WarehouseContent() {
  ...
  return (
    <>
      <GlassPanel className="flex flex-col" style={{ width: panelW, flexShrink: 0 } as any} glow={`${V.amber}04`}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
          ...
          {[
            ...
            <div key={s.label}>
              <div className="flex items-center justify-between mb-1">
                <span style={{ fontSize: 10, fontWeight: 600, color: V.textSecondary } as any}>{s.label}</span>
                <span style={{ fontFamily: V.mono, fontSize: 9, color: s.color } as any}>{s.value} / {s.max}</span>
              </div>
              <div className="h-2 rounded-full overflow-hidden" style={{ background:  "rgba(255,255,255,0.04)" } as any}>
                <motion.div initial={{ width: 0 }} animate={{ width: `${s.pct}%` }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                  className="h-full rounded-full" style={{ background:  s.color, boxShadow: `0 0 6px ${s.color}30` } as any} />
              </div>
            </div>
          ))}
          <div className="h-px" style={{ background:  V.border } as any} />
          <div className="space-y-2">
            {[
              ...
              <div key={s.label} className="flex items-center justify-between">
                <span style={{ fontSize: 10, color: V.textTertiary } as any}>{s.label}</span>
                <span style={{ fontFamily: V.mono, fontSize: 12, fontWeight: 700, color: s.color } as any}>{s.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <GlassPanel className={`flex flex-col ${selectedPkg ? "" : "flex-1"}`} style={selectedPkg ? { flex: "1 1 0%", minWidth: 0 } : {}} glow={`${V.cyan}04`}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            style={{ background:  V.cyanDim, border: `1px solid ${V.borderCyan}`, cursor: "pointer" } as any}>
            <Download size={10} color={V.cyan} />
            <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.cyan } as any}>EXPORT</span>
          </motion.button>
        </div>
        <div className="flex items-center gap-4 px-5 py-2" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
          {["PACKAGE", "VERSION", "SIZE", "FORMAT", "INTEGRITY", "VERIFIED"].map(h => (
            <span key={h} className={h === "PACKAGE" ? "flex-1" : ""} style={{ fontSize: 8, fontWeight: 700, color: V.textMuted, letterSpacing: "0.12em", minWidth: h === "PACKAGE" ? undefined : 70 } as any}>{h}</span>
          ))}
        </div>
        <StaggerList className="flex-1 overflow-y-auto">
            ...
            return (
              <StaggerItem key={pkg.id}>
                <div className="flex items-center gap-4 px-5 py-2.5 transition-colors cursor-pointer"
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background:  pkg.integrity === 100 ? V.emerald : V.amber, boxShadow: `0 0 4px ${pkg.integrity === 100 ? V.emerald : V.amber}60` } as any} />
                    <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: isSel ? 700 : 600, color: isSel ? V.cyanBright : V.textPrimary } as any}>{pkg.name}</span>
                    <span className="px-1.5 py-0.5 rounded" style={{ fontSize: 7, fontFamily: V.mono, color: V.textMuted, background: "rgba(255,255,255,0.03)" } as any}>{pkg.category}</span>
                  </div>
                  <span style={{ fontFamily: V.mono, fontSize: 10, color: V.cyanBright, minWidth: 70 } as any}>{pkg.version}</span>
                  <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textTertiary, minWidth: 70 } as any}>{pkg.size}</span>
                  <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textMuted, minWidth: 70 } as any}>{pkg.format}</span>
                  <span style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: pkg.integrity === 100 ? V.emerald : V.amber, minWidth: 70 } as any}>{pkg.integrity}%</span>
                  <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted, minWidth: 70 } as any}>{pkg.lastVerified}</span>
                </div>
              </StaggerItem>
            );
          })}
      ...
      <AnimatePresence>
        {activePkg && activeMeta && (
          ...
          >
            <GlassPanel className="flex flex-col h-full" style={{ width: 320 } as any} glow={`${V.violet}06`} borderColor={V.borderViolet}>
              {/* Inspection Header */}
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
                <div className="flex items-center gap-2">
                ...
                <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                  ...
                  style={{ color: V.textMuted, cursor: "pointer" } as any}>
                  <XCircle size={14} />
                </motion.button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span style={{ fontFamily: V.display, fontSize: 14, fontWeight: 700, color: V.textPrimary } as any}>{activePkg.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span style={{ fontFamily: V.mono, fontSize: 10, color: V.cyanBright } as any}>{activePkg.version}</span>
                    <span style={{ fontSize: 10, color: V.textMuted } as any}>by</span>
                    <span style={{ fontSize: 10, color: V.violetBright } as any}>{activeMeta.author}</span>
                  </div>
                </div>

                {/* Integrity Gauge */}
                <div className="rounded-xl p-3" style={{ background:  V.bgElevated, border: `1px solid ${V.border}` } as any}>
                  ...
                  <div className="h-2.5 rounded-full overflow-hidden" style={{ background:  "rgba(255,255,255,0.04)" } as any}>
                    <motion.div
                      initial={{ width: 0 }}
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    <span style={{ fontSize: 9, color: V.textMuted } as any}>Last verified: {activePkg.lastVerified}</span>
                    <span style={{ fontSize: 9, fontFamily: V.mono, color: activePkg.integrity === 100 ? V.emerald : V.amber } as any}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {[
                      ...
                      <div key={m.label} className="rounded-lg px-2.5 py-2" style={{ background:  V.bgElevated, border: `1px solid ${V.borderSubtle}` } as any}>
                        <span style={{ fontSize: 8, fontWeight: 700, color: V.textMuted, letterSpacing: "0.1em", textTransform: "uppercase" as const } as any}>{m.label}</span>
                        <div className="mt-0.5" style={{ fontFamily: V.mono, fontSize: 10, color: m.color } as any}>{m.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1.5">
                    {[
                      ...
                      <div key={s.label} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg" style={{ background:  V.bgElevated, border: `1px solid ${V.borderSubtle}` } as any}>
                        <span style={{ fontSize: 10, color: V.textTertiary } as any}>{s.label}</span>
                        <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: 700, color: s.color } as any}>{s.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {activeMeta.resources.map(res => {
                      ...
                      return (
                        <div key={res} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg" style={{ background:  V.bgElevated, border: `1px solid ${V.borderSubtle}` } as any}>
                          <FileCode size={10} color={resColor} />
                          <span className="flex-1 truncate" style={{ fontFamily: V.mono, fontSize: 10, color: V.textSecondary } as any}>{res}</span>
                          <span className="px-1 py-0.5 rounded" style={{ fontSize: 7, fontFamily: V.mono, color: resColor, background: `${resColor}10` } as any}>{ext.toUpperCase()}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {activeMeta.deps.map(dep => (
                      <div key={dep} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg" style={{ background:  V.bgElevated, border: `1px solid ${V.borderSubtle}` } as any}>
                        <CheckCircle2 size={10} color={V.emerald} />
                        <span className="flex-1" style={{ fontFamily: V.mono, fontSize: 10, color: V.textSecondary } as any}>{dep}</span>
                        <span style={{ fontFamily: V.mono, fontSize: 8, color: V.emerald } as any}>OK</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    ...
                    <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.cyan } as any}>RE-VERIFY</span>
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg"
                    style={{ background:  V.violetDim, border: `1px solid ${V.borderViolet}`, cursor: "pointer" } as any}>
                    <Download size={10} color={V.violet} />
                    <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.violet } as any}>EXTRACT</span>
                  </motion.button>
                </div>
              </div>
            </GlassPanel>
...
function SatelliteContent() {
  ...
  return (
    <>
      <GlassPanel className="flex flex-col" style={{ width: panelW, flexShrink: 0 } as any} glow={`${V.emerald}04`}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {[
            ...
            <div key={s.label} className="flex items-center justify-between py-2" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
              <span style={{ fontSize: 11, color: V.textSecondary } as any}>{s.label}</span>
              <span style={{ fontFamily: V.mono, fontSize: 14, fontWeight: 800, color: s.color } as any}>{s.value}</span>
            </div>
          ))}
          <div className="h-px" style={{ background:  V.border } as any} />
          ...
          {["US-EAST", "US-WEST", "EU-WEST", "EU-CENTRAL", "AP-EAST", "AP-SOUTH"].map(r => {
            ...
            return (
              <div key={r} className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full" style={{ background:  color, boxShadow: `0 0 4px ${color}60` } as any} />
                <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textTertiary } as any}>{r}</span>
                <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted, marginLeft: "auto" } as any}>{eps.length} endpoint{eps.length !== 1 ? "s" : ""}</span>
              </div>
            );
          })}
        </div>
        <div className="flex items-center justify-between px-4 py-2" style={{ borderTop: `1px solid ${V.border}` } as any}>
          <span style={{ fontSize: 9, fontFamily: V.mono, color: V.textMuted } as any}>Network: Healthy</span>
          <NeuralDot color={V.emerald} size={4} />
        </div>
      </GlassPanel>
      <div className="flex-1 flex gap-2 min-w-0">
        <GlassPanel className={`flex flex-col ${activeEp ? "w-1/2" : "flex-1"}`} glow={`${V.cyan}04`}>
          <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
            ...
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg"
              style={{ background:  V.emeraldDim, border: `1px solid ${V.borderEmerald}`, cursor: "pointer" } as any}>
              <RefreshCw size={10} color={V.emerald} />
              <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.emerald } as any}>REFRESH ALL</span>
            </motion.button>
          </div>
          <StaggerList className="flex-1 overflow-y-auto">
                  ...
                  <div className="px-5 py-3.5 transition-colors cursor-pointer" style={{
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-2 h-2 rounded-full" style={{ background:  statusColor, boxShadow: `0 0 6px ${statusColor}60` } as any} />
                        <span style={{ fontSize: 12, fontWeight: 700, color: V.textPrimary } as any}>{ep.name}</span>
                        <span className="px-1.5 py-0.5 rounded" style={{ fontSize: 8, fontFamily: V.mono, fontWeight: 700, color: statusColor, background: `${statusColor}10` } as any}>{ep.status.toUpperCase()}</span>
                      </div>
                      <span style={{ fontFamily: V.mono, fontSize: 10, fontWeight: 700, color: V.cyanBright } as any}>{ep.latency}</span>
                    </div>
                    <div className="flex items-center gap-4 ml-4">
                      <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>{ep.id}</span>
                      <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textTertiary } as any}>{ep.region}</span>
                      <span style={{ fontFamily: V.mono, fontSize: 9, color: V.violet } as any}>{ep.protocol}</span>
                      <span style={{ fontSize: 9, color: V.textMuted } as any}>Synced: {ep.lastSync}</span>
                      <span style={{ fontSize: 9, color: V.emerald, marginLeft: "auto" } as any}>{ep.uptime} uptime</span>
                    </div>
                  </div>
                </StaggerItem>
              );
          ...
          {activeEp && (
            ...
            >
              <GlassPanel className="h-full flex flex-col" glow={`${V.violet}06`} borderColor={V.borderViolet}>
                <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${V.border}`, background: "rgba(3,3,5,0.5)" } as any}>
                  <div className="flex items-center gap-2">
                    <Server size={13} color={V.violetBright} />
                    <span style={{ fontSize: 14, fontWeight: 800, color: V.textPrimary, fontFamily: V.display } as any}>{activeEp.name}</span>
                  </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span style={{ fontFamily: V.mono, fontSize: 14, fontWeight: 800, color: activeEp.status === "connected" ? V.emerald : activeEp.status === "degraded" ? V.amber : V.textTertiary } as any}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={d.label} className="rounded-lg px-3 py-2.5" style={{ background:  "rgba(255,255,255,0.02)", border: `1px solid ${V.borderSubtle}` } as any}>
                          <span style={{ fontSize: 8, fontWeight: 700, color: V.textMuted, letterSpacing: "0.12em" } as any}>{d.label}</span>
                          <div style={{ fontFamily: V.mono, fontSize: 13, fontWeight: 700, color: d.color, marginTop: 3 } as any}>{d.value}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span style={{ fontSize: 8, fontFamily: V.mono, color: V.textDim } as any}>-60s</span>
                        <span style={{ fontSize: 8, fontFamily: V.mono, color: V.textDim } as any}>now</span>
                      </div>
                    </div>

                    {/* Action buttons */}
                    <div className="flex items-center gap-2 pt-2" style={{ borderTop: `1px solid ${V.border}` } as any}>
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg flex-1 justify-center"
                        style={{ background:  V.emeraldDim, border: `1px solid ${V.borderEmerald}`, cursor: "pointer" } as any}>
                        <RefreshCw size={10} color={V.emerald} />
                        <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.emerald } as any}>PING</span>
                      </motion.button>
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg flex-1 justify-center"
                        style={{ background:  V.violetDim, border: `1px solid ${V.borderViolet}`, cursor: "pointer" } as any}>
                        <Activity size={10} color={V.violetBright} />
                        <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.violetBright } as any}>DIAGNOSE</span>
                      </motion.button>
                      {activeEp.status !== "standby" && (
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                          className="flex items-center gap-1.5 px-3 py-2 rounded-lg flex-1 justify-center"
                          style={{ background:  V.roseDim, border: `1px solid ${V.borderRose}`, cursor: "pointer" } as any}>
                          <XCircle size={10} color={V.roseBright} />
                          <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.roseBright } as any}>DISCONNECT</span>
                        </motion.button>
                      )}
                    </div>
                  </motion.div>
...
function ConfigToggle({ label, description, value, onChange, color = V.emerald }: {
  ...
  return (
    <div className="flex items-center justify-between py-3 px-1" style={{ borderBottom: `1px solid ${V.borderSubtle}` } as any}>
      <div className="flex-1 mr-4">
        <div style={{ fontSize: 12, fontWeight: 600, color: V.textPrimary } as any}>{label}</div>
        {description && <div style={{ fontSize: 10, color: V.textMuted, marginTop: 2 } as any}>{description}</div>}
      </div>
      <button onClick={() => onChange(!value)} className="relative flex-shrink-0"
        style={{ width: 36, height: 20, borderRadius: 10, background: value ? color : "rgba(255,255,255,0.08)", transition: "background 0.2s", border: `1px solid ${value ? `${color}50` : V.borderSubtle}` } as any}>
        <motion.div className="absolute top-0.5 rounded-full" animate={{ left: value ? 17 : 2 }}
          ...
          style={{ width: 16, height: 16, background: value ? "#fff" : V.textMuted, boxShadow: value ? `0 0 6px ${color}60` : "none" } as any} />
      </button>
    </div>
  );
}
                ...
                <div className="flex items-center gap-2.5 px-4 py-2.5 cursor-pointer transition-colors relative"
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      style={{ background:  V.violet, boxShadow: `0 0 6px ${V.violet}60` } as any}
                      transition={{ type: "spring", stiffness: 520, damping: 35 }}
                    />
                  )}
                  <c.icon size={13} color={isActive ? c.color : V.textMuted} />
                  <span style={{ fontSize: 12, fontWeight: isActive ? 700 : 500, color: isActive ? V.textPrimary : V.textTertiary } as any}>{c.label}</span>
                </div>
              </StaggerItem>
            );
          })}
      ...
      <GlassPanel className="flex-1 flex flex-col" glow={`${cat.color}04`}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${V.border}` } as any}>
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            style={{ background:  V.emeraldDim, border: `1px solid ${V.borderEmerald}`, cursor: "pointer" } as any}>
            <CheckCircle2 size={10} color={V.emerald} />
            <span style={{ fontFamily: V.mono, fontSize: 9, fontWeight: 700, color: V.emerald } as any}>SAVE ALL</span>
          </motion.button>
        </div>
        <AnimatePresence mode="wait">
          <motion.div
...
/* ═══════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════ */
export function RebelsVaultView({ project, activePackage: _activePackage, onNavigate: _onNavigate }: RebelsVaultViewProps) {
  const [activeTab, setActiveTab] = useState<NavTab>("triage");
  const [selectedNode, setSelectedNode] = useState(0);
  const [searchQuery, setSearchQuery] = useState("");
  const [neuralPulse, setNeuralPulse] = useState(true);

  // Sync with project data if provided
  const projectNodes = useMemo(() => {
    if (!project) return diagNodes;
    // Map real files to diagnostic nodes if no diagnostics are present
    return project.files.slice(0, 8).map((f, i) => ({
      id: `DN-${String(i + 1).padStart(3, '0')}`,
      name: f.name,
      module: f.path.split('/')[1] || "core",
      severity: (i % 3 === 0 ? "high" : i % 2 === 0 ? "medium" : "low") as any,
      conflictScore: 20 + (i * 7) % 70,
      issueCount: (i * 3) % 10,
      lastScan: "Now",
      status: "active" as const,
      description: `Analysis for ${f.name} in progress. Key resource: ${f.id}`,
      affectedFiles: [f.name],
      suggestedFix: "Run AI deep scan for detailed resolution."
    }));
  }, [project]);
  ...
  const filteredNodes = useMemo(() => {
    const sourceNodes = projectNodes;
    if (!searchQuery.trim()) return sourceNodes;
    const q = searchQuery.toLowerCase();
    return sourceNodes.filter(n =>
  ...
  return (
    <div className="flex flex-col h-full w-full select-none" style={{ background:  V.bg, fontFamily: V.sans, color: V.textPrimary } as any}>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="absolute top-0 left-0 right-0 h-px" style={{ background:  `linear-gradient(90deg, transparent, ${V.violet}40, transparent)` } as any} />
            </div>
            <div className="flex flex-col">
              ...
              <span style={{ fontSize: 9, color: V.violet, fontWeight: 600, letterSpacing: "0.15em", marginTop: 1 } as any}>
          ...
          <div className="flex items-center gap-2 ml-4 px-3 py-1.5 rounded-lg" style={{
            ...
            <span style={{ fontSize: 9, color: V.violetBright, fontWeight: 700, letterSpacing: "0.1em" } as any}>
              NEURAL ENGINE
            </span>
            <span style={{ fontSize: 9, color: V.textMuted } as any}>|</span>
            <span style={{ fontSize: 9, fontFamily: V.mono, color: neuralPulse ? V.emerald : V.textTertiary, fontWeight: 600 } as any}>
              ACTIVE
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: 700, color: V.cyan, letterSpacing: "0.06em" } as any}>SYNC</span>
          </button>
          ...
          >
            ...
            <span style={{ fontFamily: V.mono, fontSize: 11, fontWeight: 700, color: V.violetBright, letterSpacing: "0.06em" } as any}>UNLOCK VAULT</span>
          </button>
        </div>
      </div>
      ...
      {/* ═══════════════════════════════════════════════════
          TELEMETRY BAR
         ═══════════════════════════════════════════════════ */}
      <StaggerList className="flex gap-2 px-3 py-2.5 flex-shrink-0" style={{ borderBottom: `1px solid ${V.border}`, background: V.bgPanel as any } as any}>
      ...
      {/* ═══════════════════════════════════════════════════
          VAULT HERO — High-Fidelity Hero Section
         ═══════════════════════════════════════════════════ */}
      <div className="relative flex-shrink-0 overflow-hidden" style={{ height: 200 }}>
        ...
        <div className="absolute inset-0">
          <img
            src={vaultHeroImage}
            alt="Rebels Vault — Spectral Overhaul"
          ...
          <div
            ...
            style={{
              background: `linear-gradient(180deg, ${V.bgPanel} 0%, transparent 30%, transparent 70%, ${V.bgPanel} 100%)`,
            }}
          />
          <div
            className="absolute inset-0"
            style={{
              background: `linear-gradient(90deg, ${V.bg}cc 0%, transparent 40%, transparent 60%, ${V.bg}cc 100%)`,
            }}
          />
          {/* Violet tint overlay */}
          <div className="absolute inset-0" style={{ background: `${V.violet}08` }} />
        </div>
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-6">
          ...
          <div className="flex flex-col gap-2 max-w-md">
            <div className="flex items-center gap-2">
              ...
              <span
                style={{
                  ...
                  lineHeight: 1.1,
                  textShadow: `0 0 30px ${V.violet}40, 0 2px 8px rgba(0,0,0,0.6)`,
                }}
              >
                REBELS VAULT
              </span>
            ...
            >
              Centralized mod vault with spectral conflict resolution, neural scan diagnostics,
              and real-time integrity monitoring. All 1,240 mods indexed and actively monitored.
            </p>
            {/* Quick stats row */}
            <div className="flex items-center gap-4 mt-1">
              {[
                { label: "SECURE", color: V.emerald, icon: Lock },
                { label: "SCANNING", color: V.cyan, icon: Radio },
                { label: "OPTIMIZED", color: V.violetBright, icon: Zap },
              ].map((stat) => {
                const SIcon = stat.icon;
                return (
                  <div key={stat.label} className="flex items-center gap-1.5">
                    <SIcon size={10} color={stat.color} />
                    <span
                      style={{
                        fontSize: 9,
                        fontFamily: V.mono,
          ...
          <div className="flex flex-col gap-3 items-end">
            <motion.button
              whileHover={{ scale: 1.03, boxShadow: `0 0 30px ${V.violet}30, 0 4px 20px rgba(0,0,0,0.4)` }}
              whileTap={{ scale: 0.97 }}
              className="flex items-center gap-3 px-6 py-3 rounded-xl transition-all"
              style={{
                background: `linear-gradient(135deg, ${V.violet}30, ${V.cyan}20)`,
                border: `1px solid ${V.borderViolet}`,
                boxShadow: `0 0 20px ${V.violet}15, 0 4px 16px rgba(0,0,0,0.3)`,
                cursor: "pointer",
                backdropFilter: "blur(24px)",
                WebkitBackdropFilter: "blur(24px)",
              }}
            >
              <Unlock size={16} color={V.violetBright} />
              <span
                style={{
                  fontFamily: V.display,
                  fontSize: 13,
                  fontWeight: 800,
                  color: V.violetBright,
                  letterSpacing: "0.08em",
                }}
              >
                UNLOCK VAULT
              </span>
              <ArrowRight size={14} color={V.violetBright} />
            </motion.button>
            <motion.button
        ...
        {/* Top edge glow accent line */}
        <div
          className="absolute top-0 left-0 right-0 h-px"
          style={{
            background: `linear-gradient(90deg, transparent, ${V.violet}40, ${V.cyan}30, ${V.violet}40, transparent)`,
            boxShadow: `0 0 12px ${V.violet}20`,
          }}
        />
        {/* Bottom edge glow accent line */}
        <div
          className="absolute bottom-0 left-0 right-0 h-px"
          style={{
            background: `linear-gradient(90deg, transparent, ${V.cyan}30, ${V.violet}40, ${V.cyan}30, transparent)`,
            boxShadow: `0 0 12px ${V.cyan}15`,
          }}
        />
      </div>
      ...
      {/* ═══════════════════════════════════════════════════
          NAVIGATION TABS
         ═══════════════════════════════════════════════════ */}
      <div className="flex items-center gap-1 px-4 py-0 flex-shrink-0" style={{ borderBottom: `1px solid ${V.border}`, background: V.bgPanel as any } as any}>
        {navTabs.map(tab => {
        ...
        <div className="flex items-center gap-3 pr-2">
          <div className="flex items-center gap-1.5">
            <NeuralDot color={V.violet} size={4} />
            <span style={{ fontSize: 8, fontFamily: V.mono, color: V.textMuted } as any}>NE-CORE</span>
          </div>
          <div className="flex items-center gap-1.5">
            <NeuralDot color={V.cyan} size={4} />
            <span style={{ fontSize: 8, fontFamily: V.mono, color: V.textMuted } as any}>NE-SCAN</span>
          </div>
          <div className="flex items-center gap-1.5">
            <NeuralDot color={V.emerald} size={4} />
            <span style={{ fontSize: 8, fontFamily: V.mono, color: V.textMuted } as any}>NE-SYNC</span>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{ background:  "rgba(3,3,5,0.95)", borderTop: `1px solid ${V.border}` } as any}
      >
        <div className="flex items-center gap-3">
          <span style={{ fontFamily: V.mono, fontSize: 10, color: V.textMuted } as any}>CRM v4.2.0</span>
          <div className="h-2.5 w-px" style={{ background:  V.border } as any} />
          <span style={{ fontSize: 10, color: V.textTertiary } as any}>Spectral Overhaul</span>
          <div className="h-2.5 w-px" style={{ background:  V.border } as any} />
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full" style={{ background:  V.emerald, boxShadow: `0 0 4px ${V.emerald}` } as any} />
            <span style={{ fontSize: 10, color: V.emerald, fontWeight: 500 } as any}>Vault Connected</span>
          </div>
          <div className="h-2.5 w-px" style={{ background:  V.border } as any} />
          <div className="flex items-center gap-1.5">
            <NeuralDot color={V.violet} size={3} />
            <span style={{ fontSize: 10, color: V.textTertiary } as any}>Neural Engine: 3 cores active</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>1,240 mods indexed</span>
          <div className="h-2.5 w-px" style={{ background:  V.border } as any} />
          <span style={{ fontFamily: V.mono, fontSize: 9, color: V.textMuted } as any}>42 issues tracked</span>
          <div className="h-2.5 w-px" style={{ background:  V.border } as any} />
          <div className="flex items-center gap-1.5">
