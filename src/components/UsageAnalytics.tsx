...
export function UsageAnalytics({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const { isConnected, metrics } = useLiveSyncStore();

  const refresh = async () => {
    setRefreshing(true);
    await liveService.connect();
    setRefreshing(false);
    toast.success("Analytics Engine Synchronized");
  };
      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center"
          ...
          <motion.div initial={{opacity:0,scale:0.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.97,y:12}}
            ...
            style={{width:"min(1060px,97vw)",height:"min(740px,92vh)",background:T.bgElevated,border:`1px solid ${T.border}`,boxShadow:`${T.shadow2Xl},inset 0 1px 0 rgba(255,255,255,0.04)`}}>
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="p-4 rounded-xl" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="px-1.5 py-0.5 rounded" style={{fontSize:9,fontWeight:700,color:T.emerald,background:T.emeraldDim}}>+12%</span>
                  </div>
                  <div style={{fontSize:22,fontWeight:800,color:T.textPrimary,fontFamily:T.mono}}>6,036</div>
                  <div style={{fontSize:10,color:T.textMuted}}>Total Downloads</div>
                </div>
                
                <div className="p-4 rounded-xl" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  <div className="flex items-center justify-between mb-2">
                    <Users size={14} color={isConnected ? T.emerald : T.textMuted}/>
                    <span className="px-1.5 py-0.5 rounded" style={{fontSize:9,fontWeight:700,color:isConnected ? T.emerald : T.textMuted,background:isConnected ? T.emeraldDim : T.bgActive}}>
                      {isConnected ? "ACTIVE" : "OFFLINE"}
                    </span>
                  </div>
                  <div style={{fontSize:22,fontWeight:800,color:isConnected ? T.emerald : T.textPrimary,fontFamily:T.mono}}>
                    {isConnected ? "1" : "0"}
                  </div>
                  <div style={{fontSize:10,color:T.textMuted}}>Live Engine Links</div>
                </div>

                <div className="p-4 rounded-xl" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  <div className="flex items-center justify-between mb-2">
                    <Calendar size={14} color={T.textMuted}/>
                    <span className="px-1.5 py-0.5 rounded" style={{fontSize:9,fontWeight:700,color:T.emerald,background:T.emeraldDim}}>LIVE</span>
                  </div>
                  <div style={{fontSize:22,fontWeight:800,color:T.textPrimary,fontFamily:T.mono}}>{metrics.latency}ms</div>
                  <div style={{fontSize:10,color:T.textMuted}}>Engine Latency</div>
                </div>

                <div className="p-4 rounded-xl" style={{background:T.bgSurface,border:`1px solid ${T.border}`}}>
                  <div className="flex items-center justify-between mb-2">
                    <Star size={14} color={T.textMuted}/>
                    <span className="px-1.5 py-0.5 rounded" style={{fontSize:9,fontWeight:700,color:T.emerald,background:T.emeraldDim}}>+0.1</span>
                  </div>
                  <div style={{fontSize:22,fontWeight:800,color:T.textPrimary,fontFamily:T.mono}}>4.8</div>
                  <div style={{fontSize:10,color:T.textMuted}}>Brand Fidelity</div>
                </div>
              </div>
                  ...
                  <div style={{height:180}}>
                    <SafeChartContainer className="w-full h-full">
                      <BarChart data={LOCALE_DOWNLOADS} layout="vertical" margin={{top:4,right:24,bottom:4,left:4}}>
                        ...
                        <Tooltip content={<CustomTooltip/>}/>
                        <Bar key="attrfreq-bar" dataKey="count" name="Downloads" radius={[0,4,4,0]} minPointSize={0}>
