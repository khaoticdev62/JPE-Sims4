...
- Name: dialog.e2e.ts >> Dialog Component >> should close dialog with close button
- Location: src\__tests__\e2e\specs\dialog.e2e.ts:56:7
...
Call log:
  - waiting for getByTestId('dialog-content').locator('[class*="DialogClose"], button[aria-label*="Close"]').first()
